# Scores for 1990-2023 

# Dr. Julian Bernauer, MZES, June 2026
...
load("C:/Users/homeoffice/OneDrive - uni-mannheim.de/AV/powdiff_jbdev_one/powerdiffusion/service/DPD2018July.Rdata")
DPD <- DPD18 
attach(DPD)
...
# 2023: use ccodealp instead of cname 
DPD$ides <- paste(DPD$cname, DPD$eper, sep = "")
#DPD$ides <- paste(DPD$ccodealp, DPD$eper, sep = "")

#Take care of order of countries - still ok with ccodealp? 
...
DPD$eper[DPD$eper==15] <- "p"
DPD$eper[DPD$eper==16] <- "q"
DPD$eper[DPD$eper==17] <- "r"

# 2023: use ccdealp instead of cname 
DPD$ides2 <- paste(DPD$cname, DPD$eper, sep = "")
#DPD$ides2 <- paste(DPD$ccodealp, DPD$eper, sep = "")
...
# Extract and save country scores for all dimensions of power diffusion
...
DPD23_scores <- data.frame(country,acronym,id,prop_diff_mean,prop_diff_sd,dec_diff_mean,dec_diff_sd,dir_diff_mean,dir_diff_sd,pres_diff_mean,pres_diff_sd)
  
write.csv(DPD23_scores,file="DPD2023_scores.csv")

write.dta(DPD23_scores, "DPD2023_scores.dta") 

save(DPD23_scores,file="DPD2023_scores.Rdata")




# Data for electoral periods per country ...