...
# Replication code Chapter 4: Income inequality  
...
# Data with Gini and controls as decsribed in Chapter 4 of the book 
load("PDD2018_ch4_perf_inequ.Rdata")
...
for(j in 1:N){
gini[j] ~ dnorm(mu.gini[j],tau.gini)
mu.gini[j] <- alpha.gini + gamma.propp*cx1[j] + gamma.decp*cx2[j] + gamma.presp*cx3[j] + gamma.dirp*cx4[j] + gamma.propdec*cx1[j]*cx2[j] + gamma.proppres*cx1[j]*cx3[j] + gamma.com*com[j] + gamma.gdp*loggdpc[j]
}

tau.gini <- pow(sigma.gini, -2)
sigma.gini ~ dunif(0, 50)

alpha.gini ~ dnorm(0,.001)
...
cx1[52] ~ dnorm(0,1) I(0,)
cx1[53] ~ dnorm(0,1) 
cx1[54] ~ dnorm(0,1)
cx1[55] ~ dnorm(0,1)
cx1[56] ~ dnorm(0,1)

cx2[52] ~ dnorm(0,1) I(0,)
cx2[53] ~ dnorm(0,1) 
cx2[54] ~ dnorm(0,1)
cx2[55] ~ dnorm(0,1)
cx2[56] ~ dnorm(0,1)
...
cx3[53] ~ dnorm(0,1) 
cx3[54] ~ dnorm(0,1) 
cx3[55] ~ dnorm(0,1) I(0,)
cx3[56] ~ dnorm(0,1) 

cx4[52] ~ dnorm(0,1) I(0,)
cx4[53] ~ dnorm(0,1) 
cx4[54] ~ dnorm(0,1)
cx4[55] ~ dnorm(0,1)
cx4[56] ~ dnorm(0,1)
...
AVBthreel.data <- list(N=N, NT=NT, NE=NE, idn=idcc, idne=idne, par=pare, elec=elece, exeleg=exelegc, cab=cabe, fed=fedc, dec=dec, bic=bicc, const=constc, jud=judc, dir=dirc, pres=presc, gini=ginic, loggdpc=loggdpc, com=comc)

AVBthreel.parameters <- c("sigma.elec", "sigma.par", "sigma.exeleg", "sigma.dec", "sigma.dir", "alpha.exeleg", "alpha.dir", "gamma.elec1", "gamma.par1", "gamma.exeleg1", "gamma.dec2", "gamma.dir4", "tau.fed", "tau.bic", "tau.const", "tau.jud", "tau.pres", "beta.cab1", "beta.fed2", "beta.bic2", "beta.const2", "beta.jud2", "beta.pres3", "cx1", "cx2", "cx3", "cx4", "beta.prescab", "beta.dircab", "gamma.presexeleg", "acab", "adec", "ae", "ap", "a.elec1",  "a.par1", "a.dec2", "a.cab1", "tau.ac", "sigma.ac", "mu.ac", "mu.ad", "mu.ae", "mu.ap", "zex1", "alpha.gini", "gamma.propp", "gamma.decp", "gamma.presp", "gamma.dirp", "gamma.propdec","gamma.proppres","tau.gini", "sigma.gini", "gamma.gdp", "gamma.com")
...
#Figure 6.1: results income inequality  
kette <- as.matrix(samples)

agini <- kette[,"alpha.gini"]
magini <- mean(agini)
sagini <- sd(agini)
...
mcoef <- c(magini/10, mgcx1, mgcx2, mgcx3, mgcx4, mgpropdec, mgproppres, mggdp, mgpcom, stdmael1, stdmgel1, stdmap1, stdmgp1, mac1, mbc1, stdmgex1, mbf2/10, stdmadec2, stdmgdec2, mbb2, mbj2, mbc2, mbp3/5, stdmgdir4) 
scoef <- c(sagini/10, sgcx1, sgcx2, sgcx3, sgcx4, sgpropdec, sgproppres, sggdp, sgpcom, stdsael1, stdsgel1, stdsap1, stdsgp1, sac1, sbc1, stdsgex1, sbf2/10, stdsadec2, stdsgdec2, sbb2, sbj2, sbc2, sbp3/5, stdsgdir4)

m1 <- c(mcoef)
s1 <- c(scoef)

var.names <- c("Mean Gini (/10)","Proportional pow. diff.","Decentral pow. diff.","Presidential pow. diff.","Direct diff.","Prop. x dec. diff.","Prop. x pres. diff.","GDP per capita","Post-communist", "Electoral disprop.","Elect. disprop. (intra-c.)","Effective nr. of parties","Eff. nr. of part. (intra-c.)","Cabinet type","Cab. type (intra-c.)","Parliamentary power","Federalism (/10)","Decentralisation","Dec. (intra-country)","Bicameralism","Judicial review","Constitutional rigidity","Presidentialism (/5)","Direct democracy")


postscript("fig4_1.eps", width = 800, height = 800, horizontal=FALSE) 
...
plot(m1, y.axis, type = "p", axes = F, xlab = "", ylab = "", pch = 21, xlim = c(-10,10), cex=1, ylim = c(min(y.axis), max(y.axis)), main = "", bg = "black")
axis(1,at = seq(-10,10, by = 2), label = seq(-10,10, by = 2), cex.axis=.9)
...
left.side <- .5 
segments(left.side,24,left.side,16) 
segments(left.side,24,left.side+.1,24) 
segments(left.side,16,left.side+.1,16)
text(.35, 20, "Outcome Model", srt = 90, cex=1)
left.side <- .5 
...
#Figure 6.2: Interaction between proportional and decentral power diffusion for income inequality

postscript("fig4_2.eps", width = 800, height = 800, horizontal=FALSE) 

par(mar=c(3,4,0,1) + 0.1)    
par(oma=c(1,1,1,0)+.1)
par(xaxs = "i") 
par(mfrow=c(1,1))
curve(mgcx1 + mgpropdec*x, from=-1,
      to=1,ylim=c(-12,2), ylab="",  xlab="", axes=F, col="white")
axis(2, at=seq(-12, 2, by=2))
axis(1, at=seq(-1.5, 1.5, by=.5))
axis(2, at = -5, label = "Effect of Proportional Power Diffusion on Gini", las = 0, tick=F, outer=F, cex.axis=1, line=2)
...
curve(mgcx1 + mgpropdec*x, from=-1.5,
      to=1.5,ylim=c(-12,2), ylab="", xlab="", col="black", lwd=2, add=T)
box()

dev.off()

#Figure 6.3: Interaction between proportional and presidential power diffusion for income inequality

postscript("fig4_3.eps", width = 800, height = 800, horizontal=FALSE) 

par(mar=c(3,4,0,1) + 0.1)    
par(oma=c(1,1,1,0)+.1)
par(xaxs = "i") 
par(mfrow=c(1,1))
curve(mgcx1 + mgproppres*x, from=-1.5,
      to=1.5,ylim=c(-12,2), ylab="",  xlab="", axes=F, col="white")
axis(2, at=seq(-12, 2, by=2))
axis(1, at=seq(-1.5, 1.5, by=.5))
axis(2, at = -5, label = "Effect of Proportional Power Diffusion on Gini", las = 0, tick=F, outer=F, cex.axis=1, line=2)
axis(1, at = 0, label = "Presidential Power Diffusion", las = 0, tick=F, outer=F, cex.axis=1, line=2)
for (i in 100:300){
  curve(gcx1[i] + gproppres[i]*x, from=-1.5, to=1.5,add=T,
        col="grey", lwd=1, lty=3)
  #lty=2 or 3
}
abline(h=0)
